"""
test_double_top_bottom_trendfollow.py

TREND-FOLLOWING variant of Double Top/Bottom -- DISTINCT from the
original REVERSAL version (test_double_top_bottom.py). Here:

  - DOUBLE TOP forming during a DOWNTREND -> SELL (continuation:
    betting the downtrend continues after this pause).
  - DOUBLE BOTTOM forming during an UPTREND -> BUY (continuation:
    betting the uptrend continues).

This is the OPPOSITE trend condition from the reversal version,
treating the pattern as a brief pause/consolidation within an
established trend, rather than a sign of that trend ending.
...
Run this FROM INSIDE your markets/ folder:
    cd markets
    python test_double_top_bottom_trendfollow.py
"""
