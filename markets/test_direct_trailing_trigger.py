"""
test_direct_trailing_trigger.py

Tests a DIRECT-TO-TRAILING profit threshold trigger -- DISTINCT from
the breakeven-only threshold we already tested and rejected. Instead
of snapping to flat breakeven when the price threshold is hit, this
goes DIRECTLY to the same Stage3 trailing logic (recent window's
low/high), which typically locks in a stop SOMEWHERE BETWEEN
breakeven and current price -- a softer intervention than flat
breakeven, applied earlier than the candle-count trigger would allow.

Run this FROM INSIDE your markets/ folder:
    cd markets
    python test_direct_trailing_trigger.py
"""
        ...
        if open_trade is not None:
            ...
            if stage3_active:
                ...
                if profit_threshold_triggered_this_candle and not stage3_active:
                    # DIRECT-TO-TRAILING: instead of snapping to flat
                    # breakeven, immediately apply the SAME trailing
                    # logic Stage 3 normally uses (recent window's
                    # low/high) -- this typically locks in a stop
                    # somewhere BETWEEN breakeven and current price,
                    # not rigidly at entry. Deferred to take effect
                    # starting NEXT candle, same fix as before, to
                    # avoid the same-candle bug.
                    window_start = max(0, i - bs.STAGE3_WINDOW + 1)
                    if open_trade["direction"] == "BUY":
                        new_stop = min(lows[window_start: i + 1])
                        stop_loss_price = max(stop_loss_price, new_stop)
                    else:
                        new_stop = max(highs[window_start: i + 1])
...
def compound_windowed(trades, window_size, starting_balance, risk_pct):
    """
    Splits the given trades into consecutive windows of window_size,
    compounding EACH window independently from a fresh starting
    balance, to check whether a result holds up consistently across
    smaller, more recent slices -- not just on average over a large
    sample, which can be dominated by one lucky/unlucky stretch.
    """
    results = []
...
def main() -> None:
    for symbol in MARKETS_TO_CHECK:
        ...
        print(f"\n  --- LAST 100 TRADES, split into windows (baseline vs each threshold) ---")
        for window_size in (20, 10):
            print(f"\n  Window size: {window_size}")
            baseline_last100 = baseline_trades[-100:] if len(baseline_trades) >= 100 else baseline_trades
            baseline_windows = compound_windowed(baseline_last100, window_size, STARTING_BALANCE, RISK_PCT)
            print(f"    {'Variant':>20} | " + " | ".join(f"W{i+1:>6}" for i in range(len(baseline_windows))))
            row = f"    {'BASELINE':>20} | " + " | ".join(f"{w['return_pct']:>+6.1f}%" for w in baseline_windows)
            print(row)

            for threshold in THRESHOLDS_TO_TEST:
                trades = simulate_with_profit_trigger(bs, candles, trend_series, threshold)
                last100 = trades[-100:] if len(trades) >= 100 else trades
                windows = compound_windowed(last100, window_size, STARTING_BALANCE, RISK_PCT)
                label = f"threshold={threshold}%"
                row = f"    {label:>20} | " + " | ".join(f"{w['return_pct']:>+6.1f}%" for w in windows)
                print(row)
