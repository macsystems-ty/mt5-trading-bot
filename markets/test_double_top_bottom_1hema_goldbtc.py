"""
test_double_top_bottom_1hema_goldbtc.py
...
This variant uses our NORMAL 1H/EMA14 trend filter (the SAME one
every other live entry uses), instead of the original 2H/4H EMA
design specific to this pattern -- testing whether a faster, more
responsive trend definition changes the pattern's real performance.

  - DOUBLE TOP: forms during an UPTREND (per the EMA14/1H), two
    peaks at roughly the same level separated by a meaningful
...
Run this FROM INSIDE your markets/ folder:
    cd markets
    python test_double_top_bottom_1hema_goldbtc.py
"""
...
EMA_TIMEFRAMES_TO_TEST = ["1h"]  # matches our NORMAL trend filter exactly (EMA14/1H, same as every other live entry)
