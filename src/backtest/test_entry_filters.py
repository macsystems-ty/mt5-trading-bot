...
Tests two new entry filters, designed to address the recurring loss
pattern found in show_stoploss_candles.py.

FILTER A ("stalling at the level"): requires the level to have been
tested at least MIN_RETESTS times before this entry.

FILTER B ("momentum exhaustion"): requires a MEANINGFUL net move
toward the level over the MOMENTUM_LOOKBACK candles before the
retest.

Run with:
    python src/backtest/test_entry_filters.py
"""
...
MIN_RETESTS_VALUES_TO_TEST = [1, 2, 3]
MOMENTUM_LOOKBACK = 10
MIN_MOMENTUM_PCT_VALUES_TO_TEST = [0.0, 0.05, 0.1, 0.2]
...
def simulate(
    ...
    loop_start = max(
        combined.SWING_LOOKBACK * 2 + 2, combined.MAX_CANDLES_NEEDED + 1, MOMENTUM_LOOKBACK + 1
    )
    ...
    for i in range(loop_start, len(df)):
        ...
        for level in active_levels:
            ...
            if level["tested_count"] < min_retests:
                continue
            ...
            if min_momentum_pct > 0:
                lookback_start = max(0, i - MOMENTUM_LOOKBACK)
                price_then = closes[lookback_start]
                price_now = closes[i]
                if direction == "BUY":
                    momentum_pct = (price_then - price_now) / price_then * 100
                else:
...
def run_and_report(label, candles_5min, trend_series, min_retests, min_momentum_pct):
    trades = simulate(candles_5min, trend_series, min_retests, min_momentum_pct)
    ...
    print(
        f"{label:<28} | {len(trades_c):>7} | {win_rate:>8.1f}% | {total_return:>+9.3f}% | "
        f"{last_500_return:>+9.3f}%"
    )
...
def main() -> None:
    ...
    print(f"{'Config':<28} | {'Trades':>7} | {'Win Rate':>9} | {'Return':>10} | {'Last 500':>10}")
    print("-" * 78)

    print("\n--- FILTER A: minimum retests at level (Filter B disabled) ---")
    for min_retests in MIN_RETESTS_VALUES_TO_TEST:
        label = f"min_retests={min_retests}"
        run_and_report(label, candles_5min, trend_series, min_retests, 0.0)

    print("\n--- FILTER B: minimum momentum into level (Filter A disabled) ---")
    for min_momentum in MIN_MOMENTUM_PCT_VALUES_TO_TEST:
        label = f"min_momentum={min_momentum}%"
        run_and_report(label, candles_5min, trend_series, 1, min_momentum)

    print("\n--- COMBINED: best-looking values from each filter together ---")
    for min_retests in [1, 2]:
        for min_momentum in [0.0, 0.1]:
            label = f"retests={min_retests}, momentum={min_momentum}%"
            run_and_report(label, candles_5min, trend_series, min_retests, min_momentum)

    print(
        "\nFor reference, our current best (no entry filters): 5,838 trades,\n"
        "43.7% win rate, +145.252% full return."
    )
