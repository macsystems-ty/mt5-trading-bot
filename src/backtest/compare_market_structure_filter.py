"""
compare_market_structure_filter.py

Tests a NEW entry filter based on market structure: before allowing
a BUY, requires the most recent N resistance levels to be Higher
Highs AND the most recent N support levels to be Higher Lows.
Mirror logic for SELL (Lower Lows + Lower Highs).

Run with:
    python src/backtest/compare_market_structure_filter.py
"""
...
STAGE3_WINDOW = 2
STRUCTURE_N_VALUES_TO_TEST = [0, 2, 3]
...
def has_confirmed_structure(active_levels: List[dict], direction: str, n: int) -> bool:
    if n <= 0:
        return True

    resistances = [lvl for lvl in active_levels if lvl["type"] == "resistance"]
    supports = [lvl for lvl in active_levels if lvl["type"] == "support"]

    if len(resistances) < n or len(supports) < n:
        return False

    recent_resistances = sorted(resistances, key=lambda lvl: lvl["index"])[-n:]
    recent_supports = sorted(supports, key=lambda lvl: lvl["index"])[-n:]

    if direction == "BUY":
        highs_ascending = all(
            recent_resistances[i]["price"] < recent_resistances[i + 1]["price"]
            for i in range(len(recent_resistances) - 1)
        )
        lows_ascending = all(
            recent_supports[i]["price"] < recent_supports[i + 1]["price"]
        ...
        return lows_descending and highs_descending


def simulate(df: pd.DataFrame, trend_series: pd.Series, structure_n: int) -> List[Trade]:
    ...
    for i in range(loop_start, len(df)):
        ...
        if not has_confirmed_structure(active_levels, direction, structure_n):
            continue
...
def main() -> None:
    ...
    print(
        f"{'Structure N':>12} | {'Trades':>7} | {'Win Rate':>9} | {'Full Return':>12} | "
        f"{'Last 500 ($10k->)':>18}"
    )
    print("-" * 70)

    for n in STRUCTURE_N_VALUES_TO_TEST:
        trades = simulate(candles_5min, trend_series, n)
        decided = [t for t in trades if t.pct_change is not None]
        trades_c = apply_commission(decided, combined.COMMISSION_RATE_PCT)

        if not trades_c:
            print(f"{n:>12} | (no trades)")
            continue
        ...
        marker = " <- no filter (current)" if n == 0 else ""
        print(
            f"{n:>12} | {len(trades_c):>7} | {win_rate:>8.1f}% | {total_return:>+11.3f}% | "
