"""
analyze_breakeven_exit_batches.py

Same batch-of-10 win/loss and balance analysis as
analyze_last_500_trades_batches.py, but using the
breakeven-then-close-on-counter-candle exit strategy instead of our
current trailing-window exit.

Run with:
    python src/backtest/analyze_breakeven_exit_batches.py
"""
...
import test_breakeven_then_close_on_counter_candle as breakeven_exit  # noqa: E402
...
def main() -> None:
    ...
    print("Running BREAKEVEN -> CLOSE-ON-COUNTER-CANDLE exit strategy backtest ...\n")

    all_trades = breakeven_exit.simulate_breakeven_then_close(candles_5min, trend_series)
    decided_trades = [t for t in all_trades if t.pct_change is not None]
    trades_with_commission = breakeven_exit.apply_commission(decided_trades, combined.COMMISSION_RATE_PCT)
    ...
    print(
        "\nFor comparison, our CURRENT strategy (trailing-window exit) over its\n"
        "own most recent 500 trades produced: $10,000 -> $13,522.72 (+35.2%)."
    )
