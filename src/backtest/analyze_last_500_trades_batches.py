"""
analyze_last_500_trades_batches.py

Runs the combined 5-pattern strategy backtest (same validated logic
as backtester_sr_patterns_combined.py), takes the LAST 500 trades
(most recent, chronologically), groups them into consecutive batches
of 10, and reports win/loss counts per batch.

Also runs the full sequential position-sizing simulation (1% risk per
trade, $10,000 starting balance) so the batch breakdown can be shown
alongside real account balance at each point.

Run with:
    python src/backtest/analyze_last_500_trades_batches.py
"""
...
def main() -> None:
    ...
    print(f"Running combined {len(combined.SELECTED_PATTERNS)}-pattern strategy backtest ...\n")

    all_trades = combined.simulate_combined(candles_5min, trend_series)
    ...
    # Simulate ONLY the last N trades, starting FRESH from $10,000 -- this
    # shows the real, grounded account balance for this specific window,
    # not a number compounded through thousands of prior trades first.
