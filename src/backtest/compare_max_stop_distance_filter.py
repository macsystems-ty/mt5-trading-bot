"""
compare_max_stop_distance_filter.py

Tests REJECTING trades entirely when the computed initial stop
distance exceeds a maximum threshold, instead of taking the trade
with that wide stop. Motivated by a real finding: our 50 biggest
losses averaged a 0.40% stop distance (vs 0.22% for typical losses),
and NONE of them ever reached breakeven -- they went wrong
immediately and stayed wrong, with no early relief. A maximum
stop-distance filter at entry may avoid these specific bad setups
rather than trying to detect "going bad" mid-trade.

MAX_STOP_DISTANCE_PCT_VALUES_TO_TEST: None = no filter (current
behavior), then several real percentile-based cutoffs.

Run with:
    python src/backtest/compare_max_stop_distance_filter.py
"""
...
STAGE3_WINDOW = 2
MAX_STOP_DISTANCE_PCT_VALUES_TO_TEST = [None, 0.30, 0.28, 0.26, 0.25, 0.24, 0.22, 0.20, 0.18, 0.16, 0.15]
...
def simulate(
    ...
    stage3_window: int,
    max_stop_distance_pct: Optional[float],
) -> List[Trade]:
    ...
    for i in range(loop_start, len(df)):
        ...
        for level in active_levels:
            ...
            # MAX STOP DISTANCE FILTER: reject this trade entirely if the
            # computed stop is wider than our allowed maximum, instead of
            # taking the trade anyway with a wide stop.
            if max_stop_distance_pct is not None and stop_distance_pct > max_stop_distance_pct:
                continue
...
def main() -> None:
    ...
    print(
        f"Fixed: N={NUM_FAVORABLE_CANDLES_REQUIRED} favorable candles before breakeven, "
        f"Stage3 window={STAGE3_WINDOW}\n"
    )

    print(
        f"{'Max Stop %':>10} | {'Trades':>7} | {'Win Rate':>9} | {'Return (full)':>14} | "
        f"{'Last 500 ($10k->)':>18}"
    )
    print("-" * 75)

    for max_dist in MAX_STOP_DISTANCE_PCT_VALUES_TO_TEST:
        trades = simulate(
            candles_5min, trend_series, NUM_FAVORABLE_CANDLES_REQUIRED, STAGE3_WINDOW, max_dist
        )
        ...
        label = "None (current)" if max_dist is None else f"{max_dist}"
        marker = " <- current" if max_dist is None else ""
        print(
            f"{label:>10} | {len(trades_c):>7} | {win_rate:>8.1f}% | {total_return:>+13.3f}% | "
            f"${final_balance:>16,.2f}{marker}"
        )

    print(
        "\nFor reference, our current best (no max-stop filter, batch-tested):\n"
