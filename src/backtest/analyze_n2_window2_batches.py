"""
analyze_n2_window2_batches.py

Batch-of-10 win/loss and compounded balance analysis for the
N=2-favorable-candles-before-breakeven mechanism, with Stage 3 using
a 2-candle price-level trailing stop (instead of closing on the
first counter-candle) -- the configuration that tested best on full
history: 43.7% win rate, +145.252% full-history return (vs current
strategy's 35.8% win rate, +104.018%).

Run with:
    python src/backtest/analyze_n2_window2_batches.py
"""
...
import compare_n2_breakeven_with_trailing_window as n2w  # noqa: E402
...
def main() -> None:
    ...
    print(
        ...
        f"STAGE3 TRAILING (window={STAGE3_WINDOW}) exit strategy backtest ...\n"
    )

    all_trades = n2w.simulate(
        candles_5min, trend_series, NUM_FAVORABLE_CANDLES_REQUIRED, STAGE3_WINDOW
    )
    decided_trades = [t for t in all_trades if t.pct_change is not None]
    trades_with_commission = n2w.apply_commission(decided_trades, combined.COMMISSION_RATE_PCT)
    ...
    print(
        ...
        "  Current strategy (trailing window=10, no breakeven): $13,522.72 (+35.2%)\n"
        "  Breakeven N=1, close-on-counter-candle: $8,974.54 (-10.25%, a loss)\n"
        "  Breakeven N=2, close-on-counter-candle: $10,675.01 (+6.75%)"
    )
