"""
compare_initial_stop_widths.py

Tests a range of initial stop-loss distance multipliers combined
with our best exit mechanism so far (N=2 favorable candles before
breakeven, Stage 3 trailing window=2).

MULTIPLIER_VALUES_TO_TEST: 1.0 = current (full TRAILING_WINDOW-based
distance), 0.75 = three-quarters, 0.5 = half, 0.25 = quarter.

Run with:
    python src/backtest/compare_initial_stop_widths.py
"""
...
STAGE3_WINDOW = 2
MULTIPLIER_VALUES_TO_TEST = [1.0, 0.75, 0.5, 0.25]
...
def simulate(
    ...
    for i in range(loop_start, len(df)):
        ...
        for level in active_levels:
            ...
            full_stop_distance = abs(fill_price - full_initial_stop)
            scaled_distance = full_stop_distance * initial_stop_multiplier

            if direction == "BUY":
                initial_stop = fill_price - scaled_distance
            else:
                initial_stop = fill_price + scaled_distance
...
def main() -> None:
    ...
    print(
        f"{'Stop Mult.':>10} | {'Trades':>7} | {'Win Rate':>9} | {'Return (full)':>14} | "
    ...
    for mult in MULTIPLIER_VALUES_TO_TEST:
        trades = simulate(
            candles_5min, trend_series, NUM_FAVORABLE_CANDLES_REQUIRED, STAGE3_WINDOW, mult
        )
        ...
        print(
            f"{mult:>10} | {len(trades_c):>7} | {win_rate:>8.1f}% | {total_return:>+13.3f}% | "
            f"{last_500_return:>+17.3f}%{marker}"
        )

    print(
        "\nFor reference, our current best (multiplier=1.0, batch-tested):\n"
        "  5,838 trades, 46.1% win rate, $10,000 -> $13,900.20 (+39.0% compounded,\n"
        "  last 500 trades)."
    )
