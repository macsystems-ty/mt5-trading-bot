"""
diagnose_large_losses.py

Compares our BIGGEST losing trades against TYPICAL losing trades, to
check for a real, detectable early-warning signal that distinguishes
them BEFORE the stop is fully hit, and to check whether large losses
correspond to unusually WIDE stop distances.

Run with:
    python src/backtest/diagnose_large_losses.py
"""
...
def simulate_with_diagnostics(df: pd.DataFrame, trend_series: pd.Series) -> List[DiagnosticTrade]:
    ...
    for i in range(loop_start, len(df)):
        ...
        if open_trade is not None:
            ...
            if candles_held == 1 and open_trade.first_candle_low is None:
                open_trade.first_candle_low = current_low
                open_trade.first_candle_high = current_high
...
def main() -> None:
    ...
    sorted_losses = sorted(losses, key=lambda t: t.pct_change)
    biggest_losses = sorted_losses[:TOP_N_LARGEST_LOSSES]
    typical_losses = sorted_losses[TOP_N_LARGEST_LOSSES:]

    print("=" * 70)
    print(f"BIGGEST {TOP_N_LARGEST_LOSSES} LOSSES vs TYPICAL LOSSES")
    print("=" * 70)
    ...
    def summarize(group, label):
        ...
        avg_loss = sum(t.pct_change for t in group) / len(group)
        avg_stop_distance = sum(t.initial_stop_distance_pct for t in group if t.initial_stop_distance_pct) / len(group)
        avg_candles_held = sum(t.candles_held for t in group if t.candles_held is not None) / len(group)
        reached_breakeven_pct = sum(1 for t in group if t.reached_breakeven) / len(group) * 100

        first_candle_adverse_fractions = []
        for t in group:
            if t.first_candle_low is None or not t.initial_stop_distance_pct:
                continue
            if t.direction == "BUY":
                adverse_move_pct = max(0.0, (t.entry_price - t.first_candle_low) / t.entry_price * 100)
            else:
                adverse_move_pct = max(0.0, (t.first_candle_high - t.entry_price) / t.entry_price * 100)
            fraction_of_stop = adverse_move_pct / t.initial_stop_distance_pct
            first_candle_adverse_fractions.append(fraction_of_stop)

        avg_first_candle_fraction = (
            sum(first_candle_adverse_fractions) / len(first_candle_adverse_fractions)
            if first_candle_adverse_fractions else 0
        )

        print(f"\n{label} (n={len(group)}):")
        print(f"  Avg loss size: {avg_loss:.4f}%")
        print(f"  Avg initial stop distance: {avg_stop_distance:.4f}%")
        print(f"  Avg candles held: {avg_candles_held:.1f}")
        print(f"  % that reached breakeven before eventually losing: {reached_breakeven_pct:.1f}%")
        print(
            f"  Avg first-candle adverse move, as fraction of total stop distance: "
            f"{avg_first_candle_fraction:.2f} (1.0 = first candle alone covered the WHOLE stop distance)"
        )

    summarize(biggest_losses, f"BIGGEST {TOP_N_LARGEST_LOSSES} LOSSES")
    summarize(typical_losses, "ALL OTHER (TYPICAL) LOSSES")

    print("\n" + "=" * 70)
    print("STOP DISTANCE DISTRIBUTION FOR THE BIGGEST LOSSES")
    print("=" * 70)
    stop_distances = sorted(t.initial_stop_distance_pct for t in biggest_losses if t.initial_stop_distance_pct)
    if stop_distances:
        print(f"Min: {stop_distances[0]:.4f}%, Max: {stop_distances[-1]:.4f}%, "
              f"Median: {stop_distances[len(stop_distances)//2]:.4f}%")
